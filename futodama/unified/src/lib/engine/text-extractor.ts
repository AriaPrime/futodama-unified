...
// ============================================
// PDF EXTRACTION (pdf-parse + Document AI fallback)
// ============================================
...
export async function extractPdfText(buffer: Buffer): Promise<ExtractionResult> {
  ...
  try {
    ...
    if (trimmedText && trimmedText.length > MIN_TEXT_FOR_SUCCESS) {
      console.log('[PDF] pdf-parse extraction successful:', trimmedText.length, 'chars');
      return {
        text: trimmedText,
        method: 'pdfjs',
        warnings,
    ...
    // Otherwise, try OCR (probably a scanned document or complex layout)
    console.log('[PDF] Text extraction minimal (' + trimmedText.length + ' chars), trying Document AI OCR...');
    warnings.push('PDF text extraction minimal, attempting Document AI OCR');
    ...
    if (trimmedText) {
      warnings.push('Document AI OCR did not improve extraction, using original text');
      return {
        text: trimmedText,
        method: 'fallback',
        warnings,
    ...
    console.error('[PDF] pdf-parse extraction failed:', error);
    warnings.push(`PDF parsing error: ${error instanceof Error ? error.message : 'Unknown error'}`);

    // Try OCR as last resort
    console.log('[PDF] Trying Document AI OCR after pdf-parse failure...');
    const ocrText = await extractTextWithOCR(buffer);
...
export async function extractWordText(buffer: Buffer, mimeType: string): Promise<ExtractionResult> {
  ...
  try {
    ...
    // Mammoth got minimal text - try Document AI
    if (!isLegacyDoc) {
    ...
    // Try Document AI as last resort
    if (!isLegacyDoc) {
